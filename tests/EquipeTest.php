...
class EquipeTest extends TestCase{
    ...
    public function getTeamsName(){
        $equipe = obtenir_equipe_nom();

        $this->assertNotEmpty($equipe);
        $this-> assertIsArray($equipe);
        $this->assertEquals(16, count($equipe));
        foreach ($equipe as $equipe) {
            $this->assertArrayHasKey('equ_id', $equipe);
            $this->assertArrayHasKey('equ_cat', $equipe);
        }
        $this->assertContains(['equ_id' => 8, 'equ_cat' => 'U14M2'], $equipe);
        $this->expectNotToPerformAssertions();
    }
}
